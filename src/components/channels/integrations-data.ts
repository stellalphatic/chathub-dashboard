...
export const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: "Copy Instagram App ID + App Secret",
        body: "From **Meta → Settings → Basic**: copy the numeric **App ID** (same value often labeled “Instagram App ID” on the Instagram API setup screen — **not** your Instagram @handle). Copy **App Secret** (Instagram app secret / Meta app secret — one secret per app). We use the secret for **X-Hub-Signature-256** on webhooks and the App ID to exchange tokens.",
      },
      {
        title: "Instagram user access token",
        body: "Generate a **User** or **Page** access token in Meta (often 1-hour / short-lived first). Paste it below — on save we **exchange it for a long-lived token** when Meta allows (same flow as NordX-style onboarding). You can also paste a token you already extended to long-lived.",
      },
      {
    ...
    configFields: [
      {
        ...
        placeholder: "e.g. 1693937714961513",
        help: "Numeric **App ID** from Meta → **Settings → Basic** (top of the dashboard). On Instagram → API setup Meta sometimes labels this “Instagram App ID” — use that numeric ID, **not** the parent app name and **not** your Instagram username.",
        required: true,
      },
      {
        key: "igUserId",
        ...
        help: "Numeric **Instagram-scoped user ID** for the professional account (Instagram → API setup, linked account row). Leave blank and we resolve it from Graph using your token when possible.",
        required: false,
      },
    ],
    secretFields: [
      {
        ...
        type: "password",
        placeholder: "From Meta → Settings → Basic → Show",
        help: "Treat like a password. Meta signs webhooks with **X-Hub-Signature-256** using this secret. Usually the same value as “Instagram app secret” in the Instagram product UI.",
        required: true,
      },
      {
        key: "accessToken",
        ...
        type: "password",
        placeholder: "Short-lived or EAAG… long-lived",
        help: "User or Page token from Meta’s token tool. If Meta issued a **short-lived** token, we automatically request a **long-lived** token on save when exchange succeeds.",
        required: true,
      },
    ],
    ...
    configFields: [
      {
        ...
        placeholder: "From Settings → Basic",
        help: "Numeric **App ID** for this Meta app (same app that owns Messenger). Required for optional long-lived token exchange on save.",
        required: true,
      },
      {
        key: "pageId",
        ...
        help: "The Page's numeric ID (Page → About → Page Transparency). Used for routing webhook `entry.id` to this connection.",
        required: true,
      },
    ],
    secretFields: [
      {
        ...
        help: "Meta App → Settings → Basic → App Secret. Verifies **X-Hub-Signature-256** for this business’s webhooks.",
        required: true,
      },
      {
        key: "accessToken",
        ...
        type: "password",
        placeholder: "EAAG… or short-lived",
        help: "Page token from Messenger → Access Tokens. Short-lived tokens are exchanged for long-lived on save when Meta allows.",
        required: true,
      },
    ],
